...
class Node(Api):
    def __init__(self, url, session=None, verify=False, disableWarnings=True,
                 timeout=None, dmeMetaFilePath=None):
        ...
        self._dmeClassMetas = None
        if dmeMetaFilePath is not None:
            with open(dmeMetaFilePath, 'rb') as f:
                logger.debug('Loading meta information from %s',
                             dmeMetaFilePath)
                dmeMetaContents = json.loads(f.read().decode('utf-8'))
                self._dmeClassMetas = dmeMetaContents['classes']
    ...
    def useX509CertAuth(self, userName, certName, keyFile):
        with open(keyFile, 'r') as f:
            key = f.read()
        self._x509Dn = 'sys/userext/user-[{}]/usercert-[{}]'.format(userName, certName)
    ...
    def startWsListener(self, pingInt=1800):
        ...
        wst = threading.Thread(target=lambda: ws.run_forever(
            sslopt={"cert_reqs": ssl.CERT_NONE}, ping_interval=pingInt))
        wst.daemon = True
        wst.start()
    ...
    @property
    def mit(self):
        if not self._dmeClassMetas:
            refObj = self.methods.Version()
            resp = refObj.GET()
            doc = xmltodict.parse(resp.text)
            version = doc['imdata']['sysmgrShowVersion']['@nxosVersion']
            curwd = os.getcwd()
            fileName = 'dme-' + version.split()[0][:-1].replace(
                '(', '.') + '-meta.json.gz'
            fileExists = False
            try:
                pydme_home = os.environ["PYDME_HOME"]
                pydme_home += '/archive'
            except KeyError:
                pydme_home = ''
            if os.path.isfile(os.path.join(curwd, fileName)):
                fileExists = True
                logger.debug(fileName + ' exists in the current directory')
            else:
                if pydme_home:
                    if os.path.isfile(os.path.join(pydme_home, fileName)):
                        fileExists = True
                        curwd = pydme_home
                        logger.debug(fileName + ' exists in PYDME_HOME')
            if not fileExists:
                # TODO
                url = 'https://github.com/CiscoDevNet/pydme/blob/master/archive/' + fileName + '?raw=true'
                resp = requests.get(url, allow_redirects=True)
                if resp.status_code != requests.codes.ok:
                    raise ResourceError('This device version has no PyDME support')
                with open(os.path.join(curwd, fileName), 'wb') as f:
                    f.write(resp.content)
            with gzip.open(os.path.join(curwd, fileName), 'rb') as f:
                with open(os.path.join(curwd, fileName[:-3]), 'wb') as nf:
                    nf.write(f.read())
            with open(os.path.join(curwd, fileName[:-3]), 'rb') as f:
                dmeMeta = json.loads(f.read().decode('utf-8'))
                dmeClassMetas = dmeMeta['classes']
                self._dmeClassMetas = dmeMeta['classes']
        return Mo(self, 'topRoot', self._dmeClassMetas)
...
class MoIter(Api):
    def __init__(self, parentApi, className, objects, dmeClassMetas):
        self._parentApi = parentApi
        self._className = className
        self._objects = objects
        self._dmeClassMetas = dmeClassMetas
        self._dmeClassMeta = dmeClassMetas[self._className]
        self._rnFormat = self._dmeClassMeta['rnFormat']
        self._iter = itervalues(self._objects)

    def __call__(self, *args, **kwargs):
        identifiedBy = self._dmeClassMeta['identifiedBy']
        if (len(args) >= 1):
        ...
        if mo is None:
            ...
            mo = Mo(self._parentApi, self._className, self._dmeClassMetas)
            for name in identifiedBy:
...
class Mo(Api):
    def __init__(self, parentApi, className, dmeClassMetas):
        ...
        self._properties = {
            x[0]: None
            for x in self._dmeClassMeta['properties'].items()
        }
        self._rnFormat = self._dmeClassMeta['rnFormat']
    ...
    def FromDn(self, dn):
        def reductionF(acc, rn):
            ...
            className = acc._dmeClassMeta['rnMap'][rnPrefix]
    ...
    def Rn(self):
        idDict = {
            ...
            if k in self._dmeClassMeta['identifiedBy']
        }
    ...
    def IsConfigurable(self):
        return self._dmeClassMeta['isConfigurable']

    def IsConfigurableProperty(self, name):
        return (name in self._dmeClassMeta['properties'] and
                self._dmeClassMeta['properties'][name]['isConfigurable'])
    ...
    def __getattr__(self, name):
        ...
        if name in self._dmeClassMeta['contains']:
            return MoIter(self, name, self._childrenByClass[name],
                          dmeClassMetas=self._dmeClassMetas)
    ...
    def _spawnChildFromAttributes(self, className, **attributes):
        rnFormat = self._dmeClassMetas[className]['rnFormat']
...
class VersionMethod(Api):
    def __init__(self, parentApi):
        super(VersionMethod, self).__init__(parentApi=parentApi)
        self._moClassName = 'mo/sys/showversion'
...
class MethodApi(Api):
    ...
    @property
    def Version(self):
        return VersionMethod(parentApi=self)
